...
public class ClasePrueba1 {
    ...
    public void editarArchivo(String rutaArchivo) {
        ...
        } catch (IOException e) {printException( "[ ClasePrueba1 ]" , e);
            printException("[ ClasePrueba1 ]", e);
        }
    }
//bw.close() must be put before br.close()

    public static void main(String[] args) {
        ClasePrueba1 editFile = null;
        try {
            editFile = new ClasePrueba1();
            editFile.editarArchivo(FILE_PRUEBA);
        } catch (Exception e) {printException( "[ ClasePrueba1 ]" , e);
            printException("[ ClasePrueba1 ]", e);
        }
    }
